...
### Basic Publication-Quality Figure```python
...
Apply journal-specific styles using the matplotlib style files in `assets/`:```python
import matplotlib.pyplot as plt
...
For statistical plots, use seaborn with publication styling:```python
...
**Recommended: Okabe-Ito palette** (distinguishable by all types of color blindness):```python
...
**Check figure size compliance:**```python
...
**Example implementation** (see `references/matplotlib_examples.md` for complete code):```python
...
**Using seaborn for automatic confidence intervals:**```python
import seaborn as sns
...
**Using seaborn for correlation matrices:**```python
import seaborn as sns
...
**Example with statistics:**```python
...
Always apply matplotlib publication styles first, then configure seaborn:```python
import seaborn as sns
import matplotlib.pyplot as plt
from style_presets import apply_publication_style
...
**Statistical comparisons:**```python
...
**Using FacetGrid for automatic faceting:**```python
# Create faceted plot
...
**Combining seaborn with matplotlib subplots:**```python
...
Seaborn includes several colorblind-safe palettes:```python
...
- More control over figure composition```python
fig, ax = plt.subplots(figsize=(3.5, 2.5))
...
- Use `height` and `aspect` for sizing```python
g = sns.relplot(data=df, x='x', y='y', col='category', kind='scatter')
```

#### Statistical Rigor with Seaborn

Seaborn automatically computes and displays uncertainty:```python
...
**Pairwise relationships for exploratory analysis:**```python
...
**Joint distributions with marginals:**```python
...
**Issue: Legend outside plot area**```python
g = sns.relplot(...)
...
**Issue: Text too small at final size**```python
...
- Configure for publication quality:```python
fig.update_layout(
