...
The manifest MUST be in the `.codex-plugin/` directory at the plugin root. Codex CLI will not recognize plugins without this file in the correct location.
...
- Plugin identification in Codex CLI
...
When Codex CLI loads components:
...
Codex CLI validates the manifest on plugin load:
