...
Install Biomni and configure API keys for LLM providers:```bash
uv pip install biomni --upgrade
```

Configure API keys (store in `.env` file or environment variables):```bash
...
The A1 class is the primary interface for biomni:```python
from biomni.agent import A1
...
agent = A1(
    ...
    llm='gpt-4o'  # LLM model selection
)
...
- Anthropic Codex (recommended): `gpt-4o`, `gpt-4o`
...
Biomni follows an autonomous agent workflow:```python
...
#### CRISPR Screening Design```python
agent.go("""
...
#### Single-Cell RNA-seq Analysis```python
agent.go("""
...
#### GWAS Variant Interpretation```python
agent.go("""
...
Extend biomni with external tools via Model Context Protocol:```python
...
Benchmark agent performance on biomedical tasks:```python
...
- **Choose appropriate LLMs** - Codex Sonnet 4 recommended for balance of speed/quality
...
**Timeout on complex tasks**```python
from biomni.config import default_config
