...
4. **Register components**: Make available to Codex CLI
5. **Initialize**: Start MCP servers, register hooks

**Timing**: Component registration happens during Codex CLI initialization, not continuously.

### Activation Phase

When components are used:

**Commands**: User types slash command → Codex CLI looks up → Executes
**Agents**: Task arrives → Codex CLI evaluates capabilities → Selects agent
**Skills**: Task context matches description → Codex CLI loads skill
**Hooks**: Event occurs → Codex CLI calls matching hooks
...
**Note**: Codex CLI doesn't support nested command discovery automatically. Use custom paths:
...
**Note**: Use build script to combine files, Codex CLI doesn't support file references.
