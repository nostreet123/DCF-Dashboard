...
**Query a specific entity:**```python
import requests
...
**Get participating molecules in a pathway:**```python
import requests
...
### Using reactome2py Package```python
import reactome2py
...
**Retrieve analysis by token:**```python
...
Map identifiers to human pathways exclusively using the `/projection/` endpoint:```python
response = requests.post(
...
Analysis results can be visualized in the Reactome Pathway Browser by constructing URLs with the analysis token:```python
...
This skill includes `scripts/reactome_query.py`, a helper script for common Reactome operations:```bash
# Query pathway information
