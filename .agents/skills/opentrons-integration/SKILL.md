...
Every Opentrons protocol follows a standard structure:```python
...
**Loading Instruments (Pipettes):**```python
def run(protocol: protocol_api.ProtocolContext):
...
**Absorbance Plate Reader:**```python
# Initialize and read
...
**Load Liquids into Wells:**```python
...
When using multi-channel pipettes:```python
