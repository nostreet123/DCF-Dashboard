...
Complete reference for all MCP server types supported in Codex CLI plugins.
...
1. **Startup**: Codex CLI spawns process with `command` and `args`
2. **Communication**: JSON-RPC messages via stdin/stdout
3. **Lifecycle**: Process runs for entire Codex CLI session
4. **Shutdown**: Process terminated when Codex CLI exits
