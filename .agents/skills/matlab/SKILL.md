...
MATLAB operates fundamentally on matrices and arrays:```matlab
% Create matrices
...
### 3. Plotting and Visualization```matlab
% 2D Plots
x = 0:0.1:2*pi;
y = sin(x);
...
### 5. Control Flow and Functions```matlab
% Conditionals
if x > 0
    disp('positive');
elseif x < 0
...
### 6. Statistics and Data Analysis```matlab
...
### 7. Differential Equations```matlab
% ODE solving
% dy/dt = -2y, y(0) = 1
f = @(t, y) -2*y;
[t, y] = ode45(f, [0 5], 1);
...
### Pattern 1: Data Analysis Pipeline```matlab
% Load data
...
### Pattern 2: Numerical Simulation```matlab
% Parameters
...
### Pattern 3: Batch Processing```matlab
