...
For users who want to perform a standard differential expression analysis:```python
import pandas as pd
...
**Common data loading patterns:**```python
...
**Single-factor designs:**```python
design = "~condition"  # Simple two-group comparison
```

**Multi-factor designs:**```python
...
Initialize the DeseqDataSet and run the complete pipeline:```python
from pydeseq2.dds import DeseqDataSet
...
Perform Wald tests to identify differentially expressed genes:```python
from pydeseq2.ds import DeseqStats
...
Apply shrinkage to reduce noise in fold change estimates:```python
...
Save results and intermediate objects:```python
import pickle
...
Standard case-control comparison:```python
...
Testing multiple treatment groups against control:```python
dds = DeseqDataSet(counts=counts_df, metadata=metadata, design="~condition")
dds.deseq2()
...
Control for technical variation:```python
...
Include continuous variables like age or dosage:```python
...
This skill includes a complete command-line script for standard analyses:```bash
# Basic usage
...
### Identifying Significant Genes```python
...
Visualize significance vs effect size:```python
import matplotlib.pyplot as plt
import numpy as np
...
Show fold change vs mean expression:```python
plt.figure(figsize=(10, 6))
...
**Solution:** Ensure sample names match exactly```python
...
**Solution:** Check if data needs transposition```python
...
**Solution:** Remove confounded variable or add interaction term```python
# Check confounding
...
## Installation and Requirements```bash
