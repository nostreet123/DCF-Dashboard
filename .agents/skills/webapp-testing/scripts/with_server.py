...
def main():
    ...
    parser.add_argument('--cwd', action='append', dest='cwds', help='Working directory for each server (can be repeated)')
    ...
    for cmd, port, cwd in zip(args.servers, args.ports, cwds):
        ...
        if not argv:
            print(f"Error: Empty server command: {cmd!r}")
            sys.exit(1)
        servers.append({'cmd': cmd, 'argv': argv, 'port': port, 'cwd': cwd})
