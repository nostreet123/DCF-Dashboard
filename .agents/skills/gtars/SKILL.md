...
Install gtars Python bindings:```bash
uv uv pip install gtars
```

### CLI Installation

Install command-line tools (requires Rust/Cargo):```bash
...
Add to Cargo.toml for Rust projects:```toml
[dependencies]
...
Identify overlapping genomic features:```python
import gtars
...
Generate coverage tracks for visualization:```bash
...
Prepare genomic data for machine learning:```python
from gtars.tokenizers import TreeTokenizer
import gtars
...
Enable verbose logging for troubleshooting:```python
import gtars
