...
**Search Fields:**
By default, keywords are searched in both title and abstract. Customize with `--search-fields`:```python
python scripts/biorxiv_search.py \
  --keywords "AlphaFold" \
  --search-fields title \
  --days-back 365
...
**Batch Processing:**
For multiple PDFs, extract DOIs from a search result JSON and download each paper:```python
import json
...
All searches return structured JSON with the following format:```json
{
  "query": {
    "keywords": ["CRISPR"],
...
2. **Extract and review results:**```python
import json
...
3. **Download selected papers:**```python
from biorxiv_search import BioRxivSearcher
...
Track research trends by analyzing publication frequencies over time:```python
python scripts/biorxiv_search.py \
...
Monitor specific researchers' preprints:```python
...
For more complex workflows, import and use the `BioRxivSearcher` class directly:```python
...
### Custom Date Range Logic```python
...
Limit the number of results returned:```python
python scripts/biorxiv_search.py \
  --keywords "COVID-19" \
  --days-back 30 \
  --limit 50 \
...
When only metadata is needed:```python
...
Integrate search results into downstream analysis pipelines:```python
import json
import pandas as pd
