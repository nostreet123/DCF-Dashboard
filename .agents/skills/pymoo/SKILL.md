...
Pymoo uses a consistent `minimize()` function for all optimization tasks:```python
...
**1. Feasibility First (Default - Recommended)**```python
from pymoo.algorithms.moo.nsga2 import NSGA2
...
**3. Constraint as Objective**```python
...
**4. Specialized Algorithms**```python
...
**Example using Pseudo-Weights:**```python
...
**2 objectives: Scatter Plot**```python
from pymoo.visualization.scatter import Scatter
...
**Solution comparison: Petal Diagram**```python
...
### Standard operator configuration:```python
from pymoo.algorithms.soo.nonconvex.ga import GA
