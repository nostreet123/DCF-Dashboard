...
Before running any analysis, validate BAM, bigWig, and BED files using the validation script:```bash
...
For standard analyses, use the workflow generator to create customized scripts:```bash
...
**Convert BAM to normalized coverage:**```bash
bamCoverage --bam input.bam --outFileName output.bw \
    --normalizeUsing RPGC --effectiveGenomeSize 2913022398 \
    --binSize 10 --numberOfProcessors 8
```

**Compare two samples (log2 ratio):**```bash
...
**Check ChIP enrichment:**```bash
plotFingerprint -b input.bam chip.bam -o fingerprint.png \
    --extendReads 200 --ignoreDuplicates
```

**Sample correlation:**```bash
...
**Create heatmap around TSS:**```bash
# Compute matrix
...
**BAM index missing:**```bash
samtools index input.bam
```

**Out of memory:**
Process chromosomes individually using `--region`:```bash
...
Run validation script to identify issues:```bash
...
Search references using grep patterns:```bash
