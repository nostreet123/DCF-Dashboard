...
- BCG Growth-Share matrices```bash
...
Use `research-lookup` extensively to gather market data:```bash
...
Generate visuals BEFORE writing the report. Use the batch generation script:```bash
...
The `market_research.sty` package provides professional formatting. Include it in your document:```latex
...
Use colored boxes to highlight key content:```latex
