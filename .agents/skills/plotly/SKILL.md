...
Basic usage with Plotly Express (high-level API):```python
...
**Note:** Plotly Express returns graph objects Figure, so you can combine approaches:```python
...
**Subplots:** Create multi-plot figures with shared axes:```python
...
**Templates:** Apply coordinated styling:```python
...
### Scientific Data Visualization```python
import plotly.express as px
...
### Time Series and Financial```python
...
For interactive web applications, use Dash (Plotly's web app framework):```bash
uv pip install dash
``````python
import dash
