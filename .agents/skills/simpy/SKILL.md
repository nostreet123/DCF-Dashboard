...
### Basic Simulation Structure```python
import simpy
...
The simulation environment manages time and schedules events.```python
import simpy
...
Processes are defined using Python generator functions (functions with `yield` statements).```python
...
### Pattern 1: Customer-Server Queue```python
import simpy
import random
...
### Pattern 2: Producer-Consumer```python
import simpy
...
### Pattern 3: Parallel Task Execution```python
import simpy
...
Create generator functions for each process type:```python
...
Use monitoring utilities to collect data. See `references/monitoring.md` for comprehensive techniques.```python
...
Synchronize simulation with wall-clock time for hardware-in-the-loop or interactive applications. See `references/real-time.md`.```python
import simpy.rt
