...
Install the Census API:```bash
uv pip install cellxgene-census
```

For machine learning workflows, install additional dependencies:```bash
...
Always use the context manager to ensure proper resource cleanup:```python
...
**Access summary information:**```python
...
**Query cell metadata to understand available data:**```python
...
**Getting metadata separately:**```python
# Query cell metadata
cell_metadata = cellxgene_census.get_obs(
    census, "homo_sapiens",
...
For queries exceeding available RAM, use `axis_query()` with iterative processing:```python
...
**Computing incremental statistics:**```python
...
For training models, use the experimental PyTorch integration:```python
...
Seamlessly integrate Census data with scanpy workflows:```python
import scanpy as sc
...
Query and integrate multiple datasets:```python
...
Unless analyzing duplicates, always include `is_primary_data == True` in queries to avoid counting cells multiple times:```python
obs_value_filter="cell_type == 'B cell' and is_primary_data == True"
```

### Specify Census Version for Reproducibility
Always specify the Census version in production analyses:```python
census = cellxgene_census.open_soma(census_version="2023-07-25")
```

### Estimate Query Size Before Loading
For large queries, first check the number of cells to avoid memory issues:```python
# Get cell count
...
The `tissue_general` field provides coarser categories than `tissue`, useful for cross-tissue analyses:```python
# Broader grouping
...
Minimize data transfer by specifying only required metadata columns:```python
obs_column_names=["cell_type", "tissue_general", "disease"]  # Not all columns
```

### Check Dataset Presence for Gene-Specific Queries
When analyzing specific genes, verify which datasets measured them:```python
...
### Use Case 1: Explore Cell Types in a Tissue```python
with cellxgene_census.open_soma() as census:
...
### Use Case 2: Query Marker Gene Expression```python
with cellxgene_census.open_soma() as census:
    adata = cellxgene_census.get_anndata(
        census=census,
        organism="Homo sapiens",
...
### Use Case 3: Train Cell Type Classifier```python
from cellxgene_census.experimental.ml import experiment_dataloader
...
### Use Case 4: Cross-Tissue Analysis```python
with cellxgene_census.open_soma() as census:
    adata = cellxgene_census.get_anndata(
        census=census,
        organism="Homo sapiens",
