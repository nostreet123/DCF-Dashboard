...
For additional dependencies (sparse operations, clustering):```bash
...
### Loading a Benchmark Dataset```python
...
- Second row: target node indices```python
...
- No padding needed—computationally efficient```python
...
**GCNConv** (Graph Convolutional Network):```python
...
**GATConv** (Graph Attention Network):```python
...
For custom layers, inherit from `MessagePassing`:```python
...
PyG provides extensive benchmark datasets:```python
...
For datasets that fit in memory, inherit from `InMemoryDataset`:```python
...
### Loading Graphs from CSV```python
import pandas as pd
import torch
...
### Node Classification (Single Graph)```python
import torch
import torch.nn.functional as F
from torch_geometric.datasets import Planetoid
...
### Graph Classification (Multiple Graphs)```python
from torch_geometric.datasets import TUDataset
from torch_geometric.loader import DataLoader
...
For large graphs, use `NeighborLoader` to sample subgraphs:```python
...
For graphs with multiple node and edge types, use `HeteroData`:```python
from torch_geometric.data import HeteroData
...
Convert homogeneous models to heterogeneous:```python
...
Or use `HeteroConv` for custom edge-type-specific operations:```python
...
Apply transforms to modify graph structure or features:```python
...
PyG provides explainability tools to understand model predictions:```python
...
For hierarchical graph representations:```python
