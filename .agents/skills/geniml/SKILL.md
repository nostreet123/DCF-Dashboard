...
Install geniml using uv:```bash
uv uv pip install geniml
```

For ML dependencies (PyTorch, etc.):```bash
uv uv pip install 'geniml[ml]'
```

Development version from GitHub:```bash
...
### Basic Region Embedding Pipeline```python
...
### scATAC-seq Analysis Pipeline```python
import scanpy as sc
...
### Universe Building and Evaluation```bash
# Generate coverage
...
Geniml provides command-line interfaces for major operations:```bash
# Region2Vec training
