...
The ChEMBL Python client is required for programmatic access:```bash
...
**Retrieve target information:**```python
target = new_client.target
egfr = target.get('CHEMBL203')
```

**Search for specific target types:**```python
...
**Query activities for a target:**```python
activity = new_client.activity
...
**Get all activities for a compound:**```python
...
Convert results to pandas DataFrame for analysis:```python
import pandas as pd
...
The client automatically caches results for 24 hours. Configure caching:```python
...
Queries execute only when data is accessed. Convert to list to force execution:```python
...
Results are paginated automatically. Iterate through all results:```python
...
### Explore Drug Repurposing```python
# Get approved drugs
