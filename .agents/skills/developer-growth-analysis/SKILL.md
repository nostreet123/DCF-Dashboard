...
description: Analyzes your recent Codex CLI chat history to identify coding patterns, development gaps, and areas for improvement, and curates relevant learning resources from HackerNews with local-first, confirmation-gated sharing.
---
...
This skill provides personalized feedback on your recent coding work by analyzing your Codex CLI chat interactions and identifying patterns that reveal strengths and areas for growth.
...
This skill performs a seven-step analysis of your development work:

1. **Reads Your Chat History**: Accesses your local Codex CLI chat history from the past 24-48 hours to understand what you've been working on.
...
6. **Redacts Sensitive Content**: Sanitizes secrets and sensitive identifiers before any outbound sharing.

7. **Optionally Sends to Slack DMs**: Sends the redacted report only when the user explicitly confirms sharing in the current run.

## Security Defaults

- **Local-first output**: The report is presented in the CLI by default.
- **No automatic outbound sharing**: Do not send to Slack unless the user explicitly confirms.
- **Redaction required before send**: Remove likely secrets, tokens, private endpoints, and sensitive pasted snippets prior to any external post.
...
7. **Optional Send Report to Slack DMs (Explicit Confirmation Required)**

   Only send if the user explicitly confirms sharing in this run (for example: "Yes, send this redacted report to my Slack DM").
   If confirmation is missing, stop after local report delivery.

   Before sending:
   - Redact likely secrets (API keys, tokens, passwords, private URLs, credential-like strings).
   - Redact sensitive excerpts from `pastedContents` unless explicitly needed and approved.
   - Show a short redaction summary so the user can verify what will be sent.

   If confirmed, use Rube MCP to send the redacted report to the user's own Slack DMs:
   ...
   This ensures the user has a safe, redacted report in a place they check regularly.
