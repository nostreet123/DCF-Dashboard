...
For single DOIs, use the quick conversion tool:```bash
# Convert single DOI
...
For DOIs, PMIDs, arXiv IDs, or URLs:```bash
# Extract from DOI
...
**Required Fields by Type**:```bibtex
@article{citationkey,
...
Use the formatter to standardize BibTeX files:```bash
...
Complete workflow for creating a bibliography:```bash
...
4. Use `citation-management` to verify final bibliography accuracy```bash
...
### Example 1: Building a Bibliography for a Paper```bash
...
### Example 2: Converting a List of DOIs```bash
...
### Example 3: Cleaning an Existing BibTeX File```bash
...
### Example 4: Finding and Citing Seminal Papers```bash
