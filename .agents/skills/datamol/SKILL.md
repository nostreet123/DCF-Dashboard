...
Guide users to install datamol:```bash
uv pip install datamol
```

**Import convention**:```python
...
**Creating molecules from SMILES**:```python
import datamol as dm
...
**Converting molecules to SMILES**:```python
# Canonical SMILES
...
**Standardization and sanitization** (always recommend for user-provided molecules):```python
# Sanitize molecule
...
**Remote file support** (S3, GCS, HTTP):```python
...
**Computing descriptors for a single molecule**:```python
...
**Batch descriptor computation** (recommended for datasets):```python
...
**Drug-likeness filtering (Lipinski's Rule of Five)**:```python
# Filter compounds
def is_druglike(mol):
...
**Generating fingerprints**:```python
...
**Similarity calculations**:```python
...
**Extracting Murcko scaffolds**:```python
# Get Bemis-Murcko scaffold (core structure)
scaffold = dm.to_scaffold_murcko(mol)
scaffold_smiles = dm.to_smiles(scaffold)
```

**Scaffold-based analysis**:```python
...
**Scaffold-based train/test splitting** (for ML):```python
...
**BRICS fragmentation** (16 bond types):```python
# Fragment molecule
fragments = dm.fragment.brics(mol)
# Returns: set of fragment SMILES with attachment points like '[1*]CCN'
```

**RECAP fragmentation** (11 bond types):```python
fragments = dm.fragment.recap(mol)
```

**Fragment analysis**:```python
...
**Aligned visualization** (for SAR analysis):```python
...
**Highlighting substructures**:```python
...
**Conformer visualization**:```python
...
**Batch reaction application**:```python
...
### Complete Pipeline: Data Loading → Filtering → Analysis```python
import datamol as dm
import pandas as pd
...
### Structure-Activity Relationship (SAR) Analysis```python
# Group by scaffold
scaffolds = [dm.to_scaffold_murcko(mol) for mol in mols]
scaffold_smiles = [dm.to_smiles(s) for s in scaffolds]
...
### Virtual Screening Pipeline```python
...
## Integration with Machine Learning```python
# Feature generation
