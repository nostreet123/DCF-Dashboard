...
**For remote/cloud usage via Forge API:**```python
...
**Inverse folding (sequence from structure):**```python
...
**Function-conditioned generation:**```python
...
Iteratively refine protein designs using ESM3's chain-of-thought generation approach.```python
...
Process multiple proteins efficiently using Forge's async executor.```python
from esm.sdk.forge import ESM3ForgeInferenceClient
import asyncio
...
**With Flash Attention (recommended for faster inference):**```bash
uv pip install esm
uv pip install flash-attn --no-build-isolation
```

**For Forge API access:**```bash
