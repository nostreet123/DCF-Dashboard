...
**Python SDK Installation:**```python
# Stable release
...
API Key Authentication (recommended for scripts):```python
...
OAuth Client Credentials (for apps):```python
...
To register an entity directly upon creation:```python
sequence = benchling.dna_sequences.create(
    DnaSequenceCreate(
        name="My Plasmid",
        bases="ATCGATCG",
...
**Creating Notebook Entries:**```python
...
**Linking Entities to Entries:**```python
...
**Creating Workflow Tasks:**```python
...
Some operations are asynchronous and return tasks:```python
...
Use generators for memory-efficient pagination:```python
...
Use the `fields()` helper for custom schema fields:```python
