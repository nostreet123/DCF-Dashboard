...
**Example - WRONG (7-stage workflow):**```bash
...
**Example - CORRECT (3 mega-stages):**```bash
...
**Hero Image (Header):**```html
<img src="figures/hero.png" class="hero-image">
```

**Section Graphics:**```html
<div class="block">
...
**Before creating the HTML, generate all visual elements:**```bash
...
**CRITICAL: Generate SIMPLE figures with MINIMAL content.**```bash
mkdir -p figures
...
**Command Line (if Chrome available):**```bash
...
**Option 1: PDF to PPTX conversion**```bash
# Using LibreOffice
...
**Option 2: Direct PPTX creation with python-pptx**```python
...
### CSS Variables for Customization```css
/* Poster dimensions */
body {
