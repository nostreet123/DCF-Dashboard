...
DeepChem provides specialized loaders for various chemical data formats:```python
...
**Critical**: For drug discovery tasks, use `ScaffoldSplitter` to prevent data leakage from similar molecular structures appearing in both training and test sets.```python
...
#### Example: Graph Neural Networks```python
...
Quick access to 30+ curated benchmark datasets with standardized train/valid/test splits:```python
...
Leverage pretrained models for improved performance, especially on small datasets:```python
...
For evaluating a model on standard benchmarks:```python
import deepchem as dc
...
For training on custom molecular datasets:```python
import deepchem as dc
...
For leveraging pretrained models:```python
import deepchem as dc
...
Train and evaluate solubility prediction models. Works with Delaney benchmark or custom CSV data.```bash
...
Train various graph neural network architectures on molecular data.```bash
# Train GCN on Tox21
...
Fine-tune pretrained models (ChemBERTa, GROVER) on molecular property prediction tasks.```bash
...
### Pattern 1: Always Use Scaffold Splitting for Molecules```python
...
### Pattern 2: Normalize Features and Targets```python
transformers = [
...
### Pattern 4: Handle Imbalanced Data```python
...
### Pattern 5: Avoid Memory Issues```python
...
**Solution**: Ensure DeepChem is installed with required dependencies:```bash
...
Basic installation:```bash
uv pip install deepchem
```

For PyTorch models (GCN, GAT, etc.):```bash
uv pip install deepchem[torch]
```

For all features:```bash
uv pip install deepchem[all]
```
