...
Retrieve comprehensive target annotations to assess druggability and biology.```python
...
Get disease details and associated targets/drugs.```python
...
Get detailed evidence supporting a target-disease association.```python
...
Identify drugs used for a disease and their targets.```python
...
Retrieve detailed drug information including mechanisms and indications.```python
...
Find all diseases associated with a target, optionally filtering by score.```python
