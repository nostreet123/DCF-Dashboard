...
### Basic Structure Operations```python
...
### Materials Project Integration```bash
# Set up API key
...
**Using convenience methods:**```python
# Read any format
...
**Using the conversion script:**```bash
...
**Coordination environment:**```python
...
**Using the analysis script:**```bash
...
**Phase diagram construction:**```python
from mp_api.client import MPRester
...
**Using the phase diagram script:**```bash
...
**Wulff shape construction:**```python
...
**Adsorption site finding:**```python
...
### High-Throughput Structure Generation```python
...
### Band Structure Calculation Workflow```python
# 1. Relaxation
relax = MPRelaxSet(struct)
...
### Surface Energy Calculation```python
# 1. Get bulk energy
...
**Import errors**: Install missing dependencies```bash
uv pip install pymatgen[analysis,vis]
```

**API key not found**: Set MP_API_KEY environment variable```bash
export MP_API_KEY="your_key_here"
```

**Structure read failures**: Check file format and syntax```python
# Try explicit format specification
struct = Structure.from_file("file.txt", fmt="cif")
```

**Symmetry analysis fails**: Structure may have numerical precision issues```python
# Increase tolerance
from pymatgen.symmetry.analyzer import SpacegroupAnalyzer
