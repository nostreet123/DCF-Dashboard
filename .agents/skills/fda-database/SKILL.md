...
Set as environment variable:```bash
export FDA_API_KEY="your_key_here"
```

### 3. Running Examples```bash
...
Create comprehensive safety profiles combining multiple data sources:```python
...
Analyze trends over time using date ranges:```python
...
Compare multiple products side-by-side:```python
...
Link data across multiple endpoints:```python
...
All API responses follow this structure:```python
{
    "meta": {
        "disclaimer": "...",
        "results": {
...
For large result sets, use pagination:```python
...
The `FDAQuery` class includes built-in caching (enabled by default):```python
...
When counting/aggregating, use `.exact` suffix:```python
# Count exact phrases
fda.count_by_field("drug", "event",
                  search="...",
                  field="patient.reaction.reactionmeddrapt",
...
Clean and validate search terms:```python
