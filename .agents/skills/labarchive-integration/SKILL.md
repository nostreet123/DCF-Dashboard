...
Use the `scripts/setup_config.py` script to create a configuration file:```bash
python3 scripts/setup_config.py
```

This creates a `config.yaml` file with the following structure:```yaml
...
**Example using Python wrapper:**```python
...
Use the `scripts/notebook_operations.py` script:```bash
...
Use the `scripts/entry_operations.py` script:```bash
...
4. Store backups with timestamp metadata```bash
...
Install the `labarchives-py` wrapper for simplified API access:```bash
