...
**Option A: Use the analyzer script**```python
...
### Example 1: Analyzing a FASTQ file```python
...
### Example 2: Analyzing a CSV dataset```python
...
### Example 3: Analyzing microscopy data```python
...
**Solution:** Provide clear installation instructions```python
try:
    from Bio import SeqIO
except ImportError:
...
The `scripts/eda_analyzer.py` can be used directly:```bash
# Basic usage
