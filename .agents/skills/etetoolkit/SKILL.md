...
Use `scripts/tree_operations.py` for command-line tree manipulation:```bash
...
**Workflow for gene tree analysis:**```python
...
**Finding orthologs and paralogs:**```python
...
**Building taxonomy-based trees:**```python
...
**Annotating existing trees:**```python
...
**Basic visualization workflow:**```python
...
Use `scripts/quick_visualize.py` for rapid visualization:```bash
# Basic visualization
...
**Advanced visualization with faces:**```python
...
The first time NCBITaxa is instantiated, it automatically downloads the NCBI taxonomy database (~300MB) to `~/.etetoolkit/taxa.sqlite`. This happens only once:```python
from ete3 import NCBITaxa
ncbi = NCBITaxa()  # Downloads database on first run
```

Update taxonomy database:```python
...
Complete workflow from gene tree to ortholog identification:```python
...
Batch process trees for analysis:```bash
# Convert format
...
Create styled visualizations:```python
...
Process multiple trees systematically:```python
from ete3 import Tree
import os
...
Load these references when detailed information is needed:```python
...
If `tree.render()` or `tree.show()` fails with Qt-related errors, install system dependencies:```bash
# macOS
brew install qt@5
...
If database download fails or becomes corrupted:```python
from ete3 import NCBITaxa
ncbi = NCBITaxa()
...
For very large trees (>10,000 leaves), use iterators instead of list comprehensions:```python
...
Specify format when reading/writing:```python
tree = Tree("tree.nw", format=1)
tree.write(outfile="output.nw", format=5)
```

NHX (New Hampshire eXtended) format preserves custom features:```python
