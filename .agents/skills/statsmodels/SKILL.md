...
### Logistic Regression (Binary Outcomes)```python
...
### Generalized Linear Models (GLM)```python
import statsmodels.api as sm
...
Statsmodels supports R-style formulas for intuitive model specification:```python
...
### Likelihood Ratio Test (Nested Models)```python
