...
UMAP follows scikit-learn conventions and can be used as a drop-in replacement for t-SNE or PCA.```python
import umap
...
### Parameter Tuning Example```python
...
Pass target labels via the `y` parameter when fitting:```python
...
For partial labels, mark unlabeled points with `-1` following scikit-learn convention:```python
...
Train a supervised embedding on labeled data, then apply to new unlabeled data:```python
...
### Visualization After Clustering```python
...
### Integration with Machine Learning Pipelines```python
from sklearn.svm import SVC
...
**Scikit-learn compatibility:** UMAP follows standard sklearn conventions and works seamlessly in pipelines:```python
...
**Example: Exploring embedding space:**```python
import numpy as np
...
For analyzing temporal or related datasets (e.g., time-series experiments, batch data):```python
...
To ensure reproducible results, always set the `random_state` parameter:```python
reducer = umap.UMAP(random_state=42)
```
