...
Requires Python 3.11+. For cloud storage support, install additional packages:```python
uv pip install s3fs  # For S3
uv pip install gcsfs  # For Google Cloud Storage
```

### Basic Array Creation```python
import zarr
import numpy as np
...
Zarr provides multiple convenience functions for array creation:```python
# Create empty array
...
### Opening Existing Arrays```python
...
Zarr arrays support NumPy-like indexing:```python
# Write entire array
z[:] = 42
...
- **Memory consideration**: Entire chunks must fit in memory during compression```python
...
**Critical**: Chunk shape dramatically affects performance based on how data is accessed.```python
...
When arrays have millions of small chunks, use sharding to group chunks into larger storage objects:```python
...
### Configuring Compression```python
from zarr.codecs.blosc import BloscCodec
...
- **Shuffle**: Enable shuffle filter for better compression on numeric data```python
...
### Local Filesystem (Default)```python
...
### Creating and Using Groups```python
# Create root group
...
Zarr provides an h5py-compatible interface for familiar HDF5 users:```python
...
Attach custom metadata to arrays and groups using attributes:```python
...
Zarr arrays implement the NumPy array interface:```python
import numpy as np
import zarr
...
Dask provides lazy, parallel computation on Zarr arrays:```python
...
Xarray provides labeled, multidimensional arrays with Zarr backend:```python
import xarray as xr
import zarr
...
### Process-Safe Operations```python
...
For hierarchical stores with many arrays, consolidate metadata into a single file to reduce I/O operations:```python
import zarr
...
### Pattern: Time Series Data```python
...
### Pattern: Large Matrix Operations```python
import dask.array as da
...
### Pattern: Cloud-Native Workflow```python
import s3fs
import zarr
...
### Pattern: Format Conversion```python
# HDF5 to Zarr
import h5py
import zarr
...
**Diagnosis**: Check chunk size and alignment```python
...
**Solution**: Use synchronizers or ensure non-overlapping writes```python
from zarr import ProcessSynchronizer
