...
The skill generates a `.codex_resources.json` file in the current working directory containing:```json
{
...
Execute the detection script at the start of any computationally intensive task:```bash
...
After running detection, read the generated `.codex_resources.json` file to inform computational decisions:```python
...
**For parallel processing:**```python
...
The detection script requires the following Python packages:```bash
