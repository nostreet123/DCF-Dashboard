...
Use the provided `scripts/assumption_checks.py` module for automated checking:```python
...
For targeted checks, use individual functions:```python
...
#### T-Test with Complete Reporting```python
...
#### ANOVA with Post-Hoc Tests```python
import pingouin as pg
...
#### Linear Regression with Diagnostics```python
...
Most effect sizes are automatically calculated by pingouin:```python
...
Always report CIs to show precision:```python
...
Determine required sample size before data collection:```python
...
Determine what effect size you could detect:```python
