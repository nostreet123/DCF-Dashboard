...
The basic pattern for accessing any TDC dataset follows this structure:```python
...
**Example - Loading ADME data:**```python
...
Predict pharmacokinetic properties of drug molecules.```python
from tdc.single_pred import ADME
...
Predict toxicity and adverse effects of compounds.```python
...
Bioactivity predictions from screening data.```python
...
Quantum mechanical properties of molecules.```python
...
Predict binding affinity between drugs and protein targets.```python
...
Predict interactions between drug pairs.```python
...
Predict protein-protein interactions.```python
...
Generate diverse, novel molecules with desirable chemical properties.```python
from tdc.generation import MolGen
data = MolGen(name='ChEMBL_V29')
split = data.get_split()
```

Use with oracles to optimize for specific properties:```python
...
Predict reactants needed to synthesize a target molecule.```python
...
Generate molecule pairs (e.g., prodrug-drug pairs).```python
...
Retrieve train/validation/test partitions with various strategies:```python
...
Use standardized metrics for evaluation:```python
...
TDC provides 11 key processing utilities:```python
...
TDC provides 17+ oracle functions for molecular optimization:```python
from tdc import Oracle
...
### Retrieve Available Datasets```python
...
See `scripts/load_and_split_data.py` for a complete example:```python
from tdc.single_pred import ADME
from tdc import Evaluator
