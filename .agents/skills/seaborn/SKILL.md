...
- Programmatic plot generation```python
...
- `col`, `row` - Facet into multiple subplots (figure-level only)```python
...
- `multiple` - How to handle hue: "layer", "stack", "dodge", "fill"```python
...
- `kind` - Plot type for catplot: "strip", "swarm", "box", "violin", "bar", "point"```python
...
- `scatter_kws`, `line_kws` - Customize scatter and line properties```python
...
- `linewidths` - Gap between cells```python
# Correlation heatmap
corr = df.corr()
...
Create subplots based on categorical variables. Most useful when called through figure-level functions (`relplot`, `displot`, `catplot`), but can be used directly for custom plots.```python
...
Show pairwise relationships between all variables in a dataset.```python
...
Combine bivariate plot with marginal distributions.```python
...
- Integrating with existing matplotlib code```python
...
- Don't need to combine with other plot types```python
# Automatic faceting
...
Each variable is a column, each observation is a row. This "tidy" format provides maximum flexibility:```python
# Long-form structure
...
Variables are spread across columns. Useful for simple rectangular data:```python
# Wide-form structure
   control  treatment
0     10.5       12.3
1      9.8       13.1
...
**Converting wide to long:**```python
...
- `"dark"` - Dark values
- `"colorblind"` - Safe for color vision deficiency```python
...
- `"viridis"`, `"magma"`, `"plasma"` - Matplotlib perceptually uniform```python
...
- `"Spectral"` - Rainbow diverging```python
sns.heatmap(correlation_matrix, cmap='vlag', center=0)
```

### Custom Palettes```python
...
`set_theme()` controls overall appearance:```python
# Set complete theme
...
- `"ticks"` - White background with axis ticks```python
...
Always use well-structured DataFrames with meaningful column names:```python
...
### 3. Use Figure-Level Functions for Faceting```python
...
Use `hue`, `size`, and `style` to encode additional dimensions:```python
sns.scatterplot(data=df, x='x', y='y',
...
Many functions compute statistics automatically. Understand and customize:```python
...
Seaborn integrates seamlessly with matplotlib for fine-tuning:```python
...
### 7. Save High-Quality Figures```python
...
### Complex Multi-Panel Figures```python
...
### Time Series with Confidence Bands```python
...
Figure-level functions place legends outside by default. To move inside:```python
g = sns.relplot(data=df, x='x', y='y', hue='category')
g._legend.set_bbox_to_anchor((0.9, 0.5))  # Adjust position
```

### Issue: Overlapping Labels```python
...
For figure-level functions:```python
sns.relplot(data=df, x='x', y='y', height=6, aspect=1.5)
```

For axes-level functions:```python
fig, ax = plt.subplots(figsize=(10, 6))
sns.scatterplot(data=df, x='x', y='y', ax=ax)
```

### Issue: Colors Not Distinct Enough```python
...
### Issue: KDE Too Smooth or Jagged```python
# Adjust bandwidth
