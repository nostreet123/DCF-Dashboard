...
**Generate All Components** (website + poster + video):```bash
...
**Generate Poster with Custom Size**:```bash
python pipeline_all.py \
  --input-dir "path/to/paper" \
  --output-dir "path/to/output" \
  --model-choice 1 \
...
**Generate Video** (lightweight pipeline):```bash
...
**Multiple Papers** (batch processing):```bash
input/
├── paper1/
│   └── main.tex
├── paper2/
...
**Twitter/X** (English, numeric folder names):```bash
mkdir -p input/001_twitter/
# Generates English promotional content
```

**Xiaohongshu/小红书** (Chinese, alphanumeric folder names):```bash
...
### Batch Processing
Process multiple papers efficiently:```bash
