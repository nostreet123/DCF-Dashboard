...
Use the `scripts/download_cosmic.py` script to download COSMIC data files:```python
...
### 3. Working with Downloaded Data```python
import pandas as pd
...
- `sample_info` - Sample metadata and tumor information```python
...
### Cancer Gene Census
Access the expert-curated list of ~700+ cancer genes with substantial evidence of cancer involvement.```python
...
Download signature profiles for mutational signature analysis.```python
...
- `fusion_genes` - Gene fusion events```python
...
- `gene_expression` - Over/under-expression data```python
...
Access drug resistance mutation data with clinical annotations.```python
...
Specify the assembly in file paths:```python
# GRCh38 (recommended)
filepath="GRCh38/cosmic/latest/CosmicMutantExport.tsv.gz"
...
**Filter mutations by gene**:```python
import pandas as pd
...
**Identify cancer genes by role**:```python
gene_census = pd.read_csv('cancer_gene_census.csv')
...
**Extract mutations by cancer type**:```python
mutations = pd.read_csv('cosmic_mutations.tsv.gz', sep='\t', compression='gzip')
lung_mutations = mutations[mutations['Primary site'] == 'lung']
```

**Work with VCF files**:```python
import pysam
