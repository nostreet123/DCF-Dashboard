...
Always initialize the client with an email address to access the polite pool (10x rate limit boost):```python
...
Install required package using uv:```bash
...
**Use for**: Finding papers by title, abstract, or topic```python
# Simple search
...
Use the two-step pattern (entity name → ID → works):```python
...
**Manual two-step approach**:```python
...
**Use for**: Analyzing research output from universities or organizations```python
...
**Use for**: Finding influential papers in a field```python
...
**Use for**: Finding freely available research```python
...
**Use for**: Tracking research output over time```python
...
**Use for**: Comprehensive analysis of author or institution research```python
...
**Use for**: Getting information for multiple DOIs, ORCIDs, or IDs efficiently```python
dois = [
...
**Use for**: Getting representative samples for analysis```python
# Small sample
...
**Use for**: Finding papers that cite a specific work```python
# Get the work
...
**Use for**: Understanding research focus areas```python
...
**Use for**: Downloading large datasets for analysis```python
...
Add email to get 10x rate limit (1 req/sec → 10 req/sec):```python
client = OpenAlexClient(email="your-email@example.edu")
```

### Use Two-Step Pattern for Entity Lookups
Never filter by entity names directly - always get ID first:```python
# ✅ Correct
...
Always use `per-page=200` for efficient data retrieval:```python
results = client.search_works(search="topic", per_page=200)
```

### Batch Multiple IDs
Use batch_lookup() for multiple IDs instead of individual requests:```python
...
Use `sample_works()` with seed for reproducible random sampling:```python
# ✅ Correct
...
Reduce response size by selecting specific fields:```python
results = client.search_works(
    search="topic",
...
### Collaboration (AND within attribute)```python
...
Access any entity type using consistent patterns:```python
client.search_works(...)
...
Use external identifiers directly:```python
# DOI for works
