...
To ensure a GIF meets Slack's constraints, use these validators:```python
...
**To compose primitives freely, follow these patterns:**```python
...
### GIF Builder (Assembly & Optimization)```python
from core.gif_builder import GIFBuilder
...
For small GIFs like emojis, text readability is challenging. A common solution involves adding outlines:```python
...
Professional-looking GIFs often use cohesive color palettes:```python
...
Optional effects for impact moments:```python
...
Smooth motion uses easing instead of linear interpolation:```python
...
Basic drawing utilities if you need them:```python
...
### Simple Reaction (Pulsing)```python
...
### Action with Impact (Bounce + Flash)```python
...
### Combining Primitives (Move + Shake)```python
from templates.shake import create_shake_animation
...
To use this toolkit, install these dependencies only if they aren't already present:```bash
