...
**Always validate priors before fitting:**```python
with model:
...
**Use the diagnostic script:**```python
...
For continuous outcomes with linear relationships:```python
...
For grouped data (use non-centered parameterization):```python
...
For autoregressive processes:```python
...
Use LOO or WAIC for model comparison:```python
...
When models are similar, average predictions:```python
...
Default and recommended for most models:```python
idata = pm.sample(
    draws=2000,
    tune=1000,
    chains=4,
...
Fast approximation for exploration or initialization:```python
with model:
...
### Comprehensive Diagnostics```python
