...
**Single Rule Application:**```python
...
**Multiple Rules with RuleFilters:**```python
...
Filter a large compound collection to identify drug-like candidates.```python
import datamol as dm
import medchem as mc
import pandas as pd
...
Apply stricter criteria during lead optimization.```python
import medchem as mc
...
Find molecules containing specific functional groups or scaffolds.```python
import medchem as mc
