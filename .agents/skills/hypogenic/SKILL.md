...
Get started with Hypogenic in minutes:```bash
# Install the package
...
**Example (headline click prediction):**```json
{
  "headline_1": [
...
**Step 1: Setup GROBID** (first time only)```bash
...
### Basic HypoGeniC Generation```python
from hypogenic import BaseTask
...
### HypoRefine/Union Methods```python
...
### Multi-Hypothesis Inference```python
...
The `extract_label()` function is critical for parsing LLM outputs. Implement it based on your task:```python
...
Create a custom label extraction function that parses LLM outputs for your domain:```python
from hypogenic import BaseTask
...
Run hypothesis generation and inference using CLI or Python API:```bash
# CLI approach
...
Clone these repositories for ready-to-use examples:```bash
