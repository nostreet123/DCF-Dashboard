...
- **Full harness**: `npm run harness:verify` (runs invariants, bun tests, pytest, typecheck, test typecheck, convex typecheck, lint, build)
- **Lint**: `npm run lint`
...
### Canonical boundaries (post-unblock)

- **Convex server calls**: route through `app/api/_lib/convexServer.ts` only — the invariant script rejects route-local `(convexClient as any)` usage elsewhere.
- **Browser token auth**: `app/api/_lib/browserTokenAuth.ts` + `lib/browserImportTokens.ts` for import/facts/history browser reads.
- **Import context**: `lib/import/convexImportContext.ts` and `lib/import/redaction.ts` for server-side import enrichment and redaction.
- **Valuation decoders**: `lib/valuation/decoders.ts` for normalizing engine/Convex payloads.
- **AI scenario analysis**: modularized under `lib/ai/scenarioAnalysis/*` with a thin `app/api/ai/scenario-analysis/route.ts`.
- **Dashboard controller**: `lib/hooks/dashboard/*` hooks + grouped `DashboardViewModel` from `lib/dashboard/viewModel.ts`.
- **Python valuation/sync**: `python/dcf_engine/valuation_kernel.py`, `convex_transport.py`, and modular `python/damodaran_sync/sync_*.py` helpers.
