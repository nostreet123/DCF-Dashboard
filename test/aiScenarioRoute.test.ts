...
describe("AI scenario analysis route", () => {
  ...
  test("enriches the model prompt with allowlisted Convex context", async () => {
    ...
    ConvexHttpClient.prototype.query = async (name, args) => {
      ...
      if (String(name) === "valuations:get") {
        expect(args).toEqual({ syncToken: "sync-token", runId: "run-1", includeTrace: true });
        return { run: { _id: "run-1", symbol: "AAPL" }, trace: { trace: { base: {} } } };
      }
