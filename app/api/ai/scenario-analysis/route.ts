...
const compactRunSummary = (run: unknown): Record<string, unknown> | null =>
  pickFields(run, [
    "_id",
    "createdAt",
    "engineVersion",
...
const dataContract = {
  ...
  recentValuationRuns:
    "Most recent Convex valuation run summaries for the symbol, including inputs, normalized inputs, result summaries, and provenance when present.",
  latestValuationRunDetail:
    "The latest successful saved valuation run detail from Convex with trace included when available.",
...
const loadConvexAiContext = async (
  ...
  try {
    ...
    if (!importedFacts && lookup.symbol) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- avoids deep Convex type instantiation
...
const readJsonPayload = async (request: Request): Promise<unknown> => {
  ...
  if (Buffer.byteLength(text, "utf8") > getMaxPayloadBytes()) {
